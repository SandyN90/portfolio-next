...
const Nav = () => {
  ...
  const pathnaame = router.pathname;
  return( 
    ...
    backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full'>
      {navData.map((link, index)=> {
        ...
        <Link 
          key={index}
          href={link.path}
          className={`${link.path === pathnaame && 'text-accent'} relative flex items-center group hover:text-accent translation-all duration-300`}
        >
